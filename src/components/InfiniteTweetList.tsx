  ...
  return (
    <li className="flex gap-4 border-b px-4 py-4">
      ...
      <div className="flex flex-grow flex-col">
        ...
        <HeartButton onClick={handleLikeClick} isLoading={toggleLike.isLoading} likedByMe={likedByMe} likeCount={likeCount}/>
      </div>
    </li>
  )
}
...
function HeartButton({ isLoading, onClick, likedByMe, likeCount }: HeartButtonProps) {
  ...
  if (session.status !== "authenticated") {
    return (
    <div className="mb-1 bt-1 flex items-center gap-3 self-start text-gray-500">
      <HeartIcon />
      <span>{likeCount}</span>
    </div>
  )}
  return (
    <button disabled={isLoading} onClick={onClick} className={`group items-center gap-1 self-start flex transition-color duration-200 -ml-2
